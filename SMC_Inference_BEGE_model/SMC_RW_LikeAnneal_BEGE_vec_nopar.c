 ...
 * @brief Demonstration of vectorisationn and parallelisation for inference on 
...
/* series approximation @note  could be a challenge to vectorise  */
void
gser_vec(double * restrict, double * restrict, double * restrict, double * restrict);
...
double
SMC_RW_LikeAnneal(VSLStreamStatePtr, unsigned int, int,double* restrict, 
...
main(int argc, char ** argv)
{
    ...
    if (NUM_PARTICLES%VECLEN == 0)
    {
        N = NUM_PARTICLES;
    }
    else
    {
        N = ((NUM_PARTICLES / VECLEN) + 1)*VECLEN;
    }
    fprintf(stderr,"Particles: %d, Vector length: %d\n",N,VECLEN);
    ...
    SMC_RW_LikeAnneal(stream, N, ndat, rate_return, prior_l, prior_u, theta);
 ...
 * uniform priors.
 *
 * @param stream the VSL MKL RNG stream
 ...
 */
double
SMC_RW_LikeAnneal(VSLStreamStatePtr stream, unsigned int N, int ndat, 
                  ...
                  double * restrict prior_u, double* restrict theta)
{
    ...
    vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD,stream,N*NUM_PARAM,theta,0,1);
    for (int i=0;i<N;i++)
    {
        for (int j=0;j<NUM_PARAM;j++)
        {
    ...
    while (gamma_t < 1.0)
    {
        ...
        /** @todo to be honest... this makes no sense to me...
         * original MATLAB
        ...
        vdRngGaussianMV(VSL_RNG_METHOD_GAUSSIANMV_BOXMULLER2,stream,N, 
                        theta_particle_prop, NUM_PARAM, VSL_MATRIX_STORAGE_FULL,mu,T);
        /* N uniforms for accept/reject step*/
        vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD,stream,N,u,0,1);
        for (int i=0;i<N-VECLEN;i+=VECLEN)
        {
        ...
        while (belowThreshold)
        {
            ...
            sum_cond = 0;
            #pragma omp simd reduction(+:sum_cond)
            for (int i=0;i<N;i++)
            {
                sum_cond += (dist_move[i] > median_dist);
            }
    ...
    for (int i=0;i<N;i++)
    {
        for (int j=0;j<NUM_PARAM;j++)
        {
            theta[i*NUM_PARAM + j] = (prior_u[j]*exp(theta_particle[i*NUM_PARAM + j]) + prior_l[j])
                                     /(exp(theta_particle[i*NUM_PARAM + j]) + 1.0);   
